...
let midiNoteToFrequency = (midiNote: number) => 2 ** ((midiNote - 69) / 12) * 440;

function FindTimer0Values(Frequency: number) {
...
function FindTimer1Values(Frequency: number) {
    const ClockTime = 100E-9; // S/C
...
function FindTimer2Values(Frequency: number) {
    const ClockTime = 100E-9; // S/C
    const targetClocks = 1 / (ClockTime * Frequency * 2);
...
function FindTimer3Values(Frequency: number) {
    const ClockTime = 100E-9; // S/C
    const targetClocks = 1 / (ClockTime * Frequency * 2);
...
type instruction =
        { tick: number, instruction: "Note off", voice: number } |
        { tick: number, instruction: "Note on", note: number, velocity: number, voice: number } |
        { tick: number, instruction: "Note change", velocity: number, voice: number } |
        { tick: number, instruction: "Volume", Volume: number, voice: number } |
        { tick: number, instruction: "Tempo", tickLengthUs: number };

function MidiToPicInstructions(data: midi.MidiData): [string, instruction[]] {
    ...
    let rawData: Uint8Array = new Uint8Array(RawInstructions.length * 3);
